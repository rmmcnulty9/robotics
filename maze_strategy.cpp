...
void get_moves(path *r1, path *r2) {
	...
	r1->moves_x = new list<int>();
	r1->moves_y = new list<int>();
...
void try_move(path *r1, path *r2) {
	if (r1->moves_x->size() == 0) { // if there are no moves left, generate more
		get_moves(path *r1, path *r2);
		try_move(path *r1, path *r2);
	}
	else {
		move(path *r1, path *r2); // try to make a move
	}
}

void move(path *r1, path *r2) {
	if (valid_move(r1, r2)) {
		r1->curr_x = r1->moves_x->pop_front();
		r1->curr_y = r1->moves_y->pop_front();
