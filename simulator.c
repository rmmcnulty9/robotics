...
int main(int argc, char **argv) {
        ...
        filter *fir_x = firFilterCreate(coef,-11420 );
	//printf("derp %d\n", fir_x->next_sample);
        filter *fir_y = firFilterCreate(coef,-1609);
        filter *fir_theta = firFilterCreate(coef, 1.3);
        ...
        while(9==fscanf(f,"%d %d %f %d %d %d %d %d %d\n", 
          ...
          &t_left, &t_right, &t_rear)) {
             ...
             if(delta_theta<M_PI_2 && theta>0 && prev_theta<0){
              theta-=(2*M_PI);
            }else if(delta_theta>M_PI_2 && theta<0 && prev_theta>0){
              theta+=(2*M_PI);
            } 
            
          /*  if(delta_theta<(3*M_PI/2)){
              delta_theta = (2*M_PI)+delta_theta;
            }else if(delta_theta>(-3*M_PI/2)){
              delta_theta = (-2*M_PI)+delta_theta;
            }
            */
              prev_theta=theta;
            ...
            total_theta += delta_theta;
               ...
               f_theta = firFilter(fir_theta, total_theta);
